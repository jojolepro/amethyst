...
use amethyst_core::{
	Time,
	specs::{Component, System, DenseVecStorage, Read, WriteStorage, Entities, Join},
};
...
impl<'a> System<'a> for BlinkSystem {
	...
	fn run(&mut self, (entities, mut hiddens, mut blinks, time): Self::SystemData){
		...
		for (entity, blink) in (&*entities, &mut blinks).join() {
			...
			if on {
				if !hiddens.contains(entity) {
					hiddens.insert(entity, Hidden).expect(&format!("Failed to insert Hidden component for {:?}", entity));
				}
			} else {
				if hiddens.contains(entity) {
