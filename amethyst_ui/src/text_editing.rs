...
pub fn build_text_editing_input_system(mut reader: ReaderId<Event>) -> impl Runnable {
    SystemBuilder::new("TextEditingInputSystem")
        ...
        .build(
            move |_commands, world, (events, mut edit_events), (texts, editables)| {
            ...
            for event in events.read(&mut reader) {
                ...
                if let Some((entity, ref mut focused_text, ref mut focused_edit, _)) = editables.iter_mut(world).next() {
                    match *event {
                        Event::WindowEvent {
